...
class Button_Group extends Extras_Widget {
			...
			if ( settings.buttons ) {
				...
				_.each( settings.buttons, function( item ) {
					...
					<li class="{{ item_class }} elementor-repeater-item-{{ item._id }}">
						<a data-hotips-position="{{ item.tooltip_position }}" data-hotips-content="#hotip-content-{{ item._id }}" id="{{ item._element_id }}" class="{{ wrapper_class }} {{ item.css_classes }} elementor-animation-{{ item.hover_animation }}" href="{{ item.link.url }}">

							<span data-label="{{{ button_text_clone }}}" class="{{ button_class }} ee-button-{{ item.button_type }} ee-size-{{ item.size }}">
	...
	public function wpml_widgets_to_translate_filter( $widgets ) {
		...
		$widgets[ $this->get_name() ] = [
			...
			'fields'     		=> [],
			'integration-class' => '\ElementorExtras\Modules\Buttons\Widgets\Buttons\WPML',
		];
