...
class Gallery extends Extras_Widget {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'fields' 		=> array_values( $gallery->get_controls() ),
					'condition'		=> [
						'gallery_type'		=> 'manual',
					]
				]
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'selectors'			=> [
						'{{WRAPPER}} .ee-grid__item--wide' => 'width: calc( 100% / {{columns.VALUE}} * 2 );',
						'{{WRAPPER}} .ee-grid__item--tall .ee-gallery__media:before' => 'padding-bottom: 150%',
					]
				]
			);
		...
		foreach ( $gallery as $item ) {
			...
			if ( '' !== $settings['gallery_link'] ) {
				...
				if ( 'file' === $settings['gallery_link'] ) {
					...
					$this->add_render_attribute( 'gallery-media-' . $counter , [
				...
				$this->add_render_attribute( 'gallery-media-' . $counter, 'href', $item_link );
			}
			...
			<div <?php echo $this->get_render_attribute_string( 'gallery-item' ); ?>>
				...
				<?php endif; ?>
					...
					<<?php echo $media_tag; ?> <?php echo $this->get_render_attribute_string( 'gallery-media-' . $counter ); ?>>
						...
						<?php $this->render_image_thumbnail( $item ); ?>
				...
				if ( 'file' === $item['link'] ) {
					...
					if ( ! empty( $item['link_url']['url'] ) ) {
						...
						if ( ! empty( $item['link_url']['is_external'] ) ) {
							$this->add_render_attribute( 'gallery-media-' . $counter, 'target', '_blank' );
						}

						if ( ! empty( $item['link_url']['nofollow'] ) ) {
							$this->add_render_attribute( 'gallery-media-' . $counter, 'rel', 'nofollow' );
						}
					}
	...
	protected function render_gallery_sizer() {
		...
		if ( 'yes' === $settings['masonry_enable'] && 'yes' !== $settings['parallax_enable'] ) : ?>
			...
			<div class="ee-grid__item ee-grid__item--sizer"></div>

		<?php endif;
	}

	protected function render_image_thumbnail( $item ) {
