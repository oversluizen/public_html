...
class EVO_Product
{
    ...
    public function in_plugin_update_message($plugin_data, $response)
    {
        ...
        if ($this->is_evo()) {
            ...
            echo '<br/><b>NOTE:</b> '.sprintf(' ' . __('If you are unable to auto update please visit %s to learn how to update manually.', 'eventon'), $redirect); else:
    ...
    // Transients
    public function force_package_update($action)
    {
        ...
        if ($this->get_prop('version') &&
            ...
            if ($action == 'append') {
                if (!$this->is_transient_ready_for_update()) {
                    return false;
                }
    ...
    public function is_transient_ready_for_update()
    {
        ...
        if (($last_update + (3 * 86400)) < (current_time('timestamp'))) {
            return true;
        }
        return false;
    }
        ...
        if ($data) {
            ...
            foreach (array('version','plugin_slug','guide_file', 'ID') as $field) {
                ...
                if (!isset($data[$field]) && isset($args[$field])) {
                    $this->set_prop($field, $args[$field]);
                    $new_data = true;
                    continue;
                }
    ...
    public function plugindata_from_file()
    {
        if (file_exists(AJDE_EVCAL_DIR.'/'.$this->slug.'/'.$this->slug.'.php')) {
            ...
            foreach (array(
                ...
                if (!isset($this->prod_loc[$field])) {
                    continue;
                }

                if ($this->get_prop($field)) {
                    continue;
                }
    ...
    public function set_prop($prop, $value, $update = true)
    {
    ...
    {
        if (!$this->get_prop('remote_version')) {
            return false;
        }
        if (!$this->get_prop('version')) {
            return false;
    ...
    {
        if (!$this->get_prop('remote_validity')) {
            false;
        }
    ...
    public function get_remote_version()
    {
        ...
        if (empty($version) && empty($remote_version)) {
            return false;
        }
    ...
    public function is_installed()
    {
        ...
        if (in_array($this->prod['plugin_slug'], $active_plugins)) {
            return true;
        }
